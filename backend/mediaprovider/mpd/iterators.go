...
func (it *albumIterator) loadAlbums() {
	...
	// Retry up to 3 times if database is being updated
...
func (it *artistIterator) loadArtists() {
	...
	for i := 0; i < maxRetries; i++ {
		...
		if err == nil || !strings.Contains(err.Error(), "database was updated") {
			break
		}
		if i < maxRetries-1 {
			log.Printf("database update detected while loading artists, retrying (%d/%d)...", i+1, maxRetries)
		}
	}
	if err != nil {
...
func (it *searchArtistIterator) loadArtists() {
	...
	for i := 0; i < maxRetries; i++ {
		...
		if i < maxRetries-1 {
			log.Printf("database update detected while loading artists for search, retrying (%d/%d)...", i+1, maxRetries)
		}
	}
	if err != nil {
		log.Printf("error loading artists: %v", err)
